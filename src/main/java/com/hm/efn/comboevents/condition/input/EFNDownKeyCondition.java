...
public class EFNDownKeyCondition implements Condition<PlayerPatch<?>> {
   private boolean checkRelease;

   public EFNDownKeyCondition() {
      this(false);
   }

   public EFNDownKeyCondition(boolean checkRelease) {
   ...
   public boolean predicate(PlayerPatch<?> playerPatch) {
      ...
      if (playerPatch.isLogicalClient()) {
         boolean currentState = GLFW.glfwGetKey(Minecraft.getInstance().getWindow().getWindow(), Minecraft.getInstance().options.keyDown.getKey().getValue()) == 1;
      ...
      if (!dataManager.hasData(EFNSKillDataKeys.DOWN_KEY)) {
         return false;
      }

      boolean isPressed = (Boolean)dataManager.getDataValue(EFNSKillDataKeys.DOWN_KEY);
