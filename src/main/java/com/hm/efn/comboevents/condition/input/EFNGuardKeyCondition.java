...
public class EFNGuardKeyCondition implements Condition<PlayerPatch<?>> {
   private boolean checkRelease;

   public EFNGuardKeyCondition() {
      this(false);
   }

   public EFNGuardKeyCondition(boolean checkRelease) {
   ...
   public boolean predicate(PlayerPatch<?> playerPatch) {
      ...
      if (!dataManager.hasData(EFNSKillDataKeys.GUARD_KEY)) {
         return false;
      }

      boolean isPressed = (Boolean)dataManager.getDataValue(EFNSKillDataKeys.GUARD_KEY);
