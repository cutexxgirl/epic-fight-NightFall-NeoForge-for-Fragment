...
public class EFNKey2Condition implements Condition<PlayerPatch<?>> {
   private boolean checkRelease;

   public EFNKey2Condition() {
      this(false);
   }

   public EFNKey2Condition(boolean checkRelease) {
   ...
   public boolean predicate(PlayerPatch<?> playerPatch) {
      ...
      if (playerPatch.isLogicalClient()) {
         boolean currentState = GLFW.glfwGetKey(Minecraft.getInstance().getWindow().getWindow(), InvincibleKeyMappings.KEY2.getKey().getValue()) == 1;
      ...
      if (!dataManager.hasData(EFNSKillDataKeys.KEY2_PRESS)) {
         return false;
      }

      boolean isPressed = (Boolean)dataManager.getDataValue(EFNSKillDataKeys.KEY2_PRESS);
