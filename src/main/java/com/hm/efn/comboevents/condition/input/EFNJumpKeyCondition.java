...
public class EFNJumpKeyCondition implements Condition<PlayerPatch<?>> {
   private boolean checkRelease;

   public EFNJumpKeyCondition() {
      this(false);
   }

   public EFNJumpKeyCondition(boolean checkRelease) {
   ...
   public boolean predicate(PlayerPatch<?> playerPatch) {
      ...
      if (playerPatch.isLogicalClient()) {
         boolean currentState = GLFW.glfwGetKey(Minecraft.getInstance().getWindow().getWindow(), Minecraft.getInstance().options.keyJump.getKey().getValue()) == 1;
      ...
      if (!dataManager.hasData(EFNSKillDataKeys.JUMP_KEY)) {
         return false;
      }

      boolean isPressed = (Boolean)dataManager.getDataValue(EFNSKillDataKeys.JUMP_KEY);
