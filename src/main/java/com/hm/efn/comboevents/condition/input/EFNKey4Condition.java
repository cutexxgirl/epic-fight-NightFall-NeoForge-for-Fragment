...
public class EFNKey4Condition implements Condition<PlayerPatch<?>> {
   private boolean checkRelease;

   public EFNKey4Condition() {
      this(false);
   }

   public EFNKey4Condition(boolean checkRelease) {
   ...
   public boolean predicate(PlayerPatch<?> playerPatch) {
      ...
      if (!dataManager.hasData(EFNSKillDataKeys.KEY4_PRESS)) {
         return false;
      }

      boolean isPressed = (Boolean)dataManager.getDataValue(EFNSKillDataKeys.KEY4_PRESS);
