...
public class MurasamaPhase extends AvalonPhase {
   ...
   public void playHitAnimation(LivingEntityPatch<?> targetPatch, LivingEntityPatch<?> attackerPatch) {
      if (!this.shouldSkipHit(targetPatch, attackerPatch)) {
         ...
         if (animationPlayer != null) {
            ...
            boolean isWhitelisted = entityId != null && ((List)EFNCommonConfig.MURASAMA_STUNANIMATION_WHITELIST.get()).contains(entityId.toString());
            if (!isWhitelisted) {
         ...
         if (animationSupplier != null && animationSupplier.get() != null) {
            targetPatch.playAnimationSynchronized(animationSupplier.get().getAccessor(), 0.07F);
         }
      }
   }
               ...
               if (!target.isInvulnerable()
                  ...
                  if (!(target instanceof Player player && (player.isCreative() || player.isSpectator()))) {
                     ...
                     if (entityId != null) {
                        ...
                        if (((List)EFNCommonConfig.MURASAMA_STUNANIMATION_BLACKLIST.get()).contains(entityIdString)) {
                           return true;
                        }

                        if (((List)EFNCommonConfig.MURASAMA_STUNANIMATION_WHITELIST.get()).contains(entityIdString)) {
                           return false;
                        }
                     }
