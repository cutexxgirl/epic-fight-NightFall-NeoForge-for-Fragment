...
public class MurasamaPassive extends PassiveSkill {
   private static final UUID EVENT_UUID = UUID.fromString("051a9bb2-7541-11ee-b962-0242ac120003");
   private static final UUID RESET_EVENT_UUID = UUID.fromString("051a9bb2-7541-11ee-b962-0721ac120003");
   private static final UUID SWORDOUT_EVENT_HURT_UUID = UUID.fromString("051a9bb2-7541-22ee-b962-0731ac110001");
   private static final UUID SWORDOUT_EVENT_GUARD_UUID = UUID.fromString("051a9bb2-7541-22ee-b962-0721ac120003");
   private static final UUID SWORDOUT_EVENT_DODGE_UUID = UUID.fromString("051a9bb2-7541-33ee-b962-0712ac120003");
   private static final UUID SHEATH_DAMAGE_EVENT_UUID = UUID.fromString("051a9bb2-7541-33ee-b962-0721ac120003");
   private static final UUID SHEATH_RESETEVENT_UUID = UUID.fromString("051a9bb2-7541-11ee-b962-0212ac120003");
   private static final UUID SPRINT_SHEATH_UUID = UUID.fromString("051a9bb2-7541-44ee-b962-0721ac120003");
   ...
   public MurasamaPassive(SkillBuilder<?> builder) {
      super(builder);
   }
   ...
   private void setupEventListeners(SkillContainer container) {
      ...
      com.hm.efn.util.EFNEventBridge.addEventListener(listener, EventType.ANIMATION_BEGIN_EVENT, SHEATH_RESETEVENT_UUID, event -> {
         if (event.getAnimation().getRealAnimation().equals(EFNMurasamaAnimations.HF_MURASAMA_KICK_Y)) {
         ...
         if (event.getAnimation().getRealAnimation().equals(EFNMurasamaAnimations.HF_MURASAMA_SWORD_OUT)) {
            container.getDataManager().setDataSync(EFNSKillDataKeys.MANUAL_UNSHEATH, true);
         }

         if (event.getAnimation().getRealAnimation().equals(EFNMurasamaAnimations.HF_MURASAMA_GUARD_DASH)) {
            container.getDataManager().setDataSync(EFNSKillDataKeys.MANUAL_UNSHEATH, true);
            this.murasamaSwordOut(container);
         }

         if (event.getAnimation().getRealAnimation().equals(EFNMurasamaAnimations.HF_MURASAMA_TAUNT)) {
            container.getDataManager().setDataSync(EFNSKillDataKeys.MANUAL_UNSHEATH, true);
            this.murasamaSwordOut(container);
         }

         if (event.getAnimation().getRealAnimation().equals(EFNMurasamaAnimations.HF_MURASAMA_JUMP_SECOND)) {
            container.getDataManager().setDataSync(EFNSKillDataKeys.MANUAL_UNSHEATH, true);
            this.murasamaSwordOut(container);
         }
         ...
         if (isSprinting) {
            ...
            if (sprintTimer >= 3) {
               ...
               if (animationPlayer != null) {
                  ...
                  isAlreadyPlaying = currentAnimation.equals(EFNMurasamaAnimations.HF_MURASAMA_SHEATH_IN_RUN.get());
               }

               if (!isAlreadyPlaying) {
                  playerPatch.playAnimationSynchronized(EFNMurasamaAnimations.HF_MURASAMA_SHEATH_IN_RUN, 0.0F);
               }
   ...
   private void handleMovementInputEvent(SkillContainer container, MovementInputEvent event) {
      MurasamaPassive.MovementInputState inputState = this.getMovementInputState(container, event);
   ...
   private MurasamaPassive.MovementInputState getMovementInputState(SkillContainer container, MovementInputEvent event) {
      ...
      return new MurasamaPassive.MovementInputState(upPressed, downPressed, jumpPressed, isSprinting, upPressedPrev, downPressedPrev, jumpPressedPrev);
   }

   private void handleDoubleTapSprint(SkillContainer container, MurasamaPassive.MovementInputState inputState, MovementInputEvent event) {
      boolean isDoubleTapSprint = (Boolean)container.getDataManager().getDataValue(EFNSKillDataKeys.IS_DOUBLE_TAP_SPRINT);
   ...
   private void handleDoubleJump(SkillContainer container, MurasamaPassive.MovementInputState inputState, MovementInputEvent event) {
   ...
   private void updateKeyStates(SkillContainer container, MurasamaPassive.MovementInputState inputState) {
   ...
   private void executeDoubleJump(SkillContainer container, MovementInputEvent event) {
      ...
      if (!skillCastEvent.isCanceled()) {
         ...
         ((LocalPlayerPatch)event.getPlayerPatch()).playAnimationSynchronized(EFNMurasamaAnimations.HF_MURASAMA_JUMP_SECOND, 0.1F);
      }
   }
   ...
   public void updateContainer(SkillContainer container) {
      ...
      if (!container.getExecutor().isLogicalClient()) {
         ...
         if (currentAnim.equals(EFNDodgeAnimations.MURASAMA_ROLL_F_AIR)) {
            this.updateComboNode((ServerPlayer)container.getServerExecutor().getOriginal(), Murasama.Air_Dodge);
         }
      }
   }
   ...
   private void updateDoubleTapSprintState(SkillContainer container) {
      ...
      boolean wasDoubleTapSprint = (Boolean)container.getDataManager().getDataValue(EFNSKillDataKeys.WAS_DOUBLE_TAP_SPRINT);
      if (isDoubleTapSprint != wasDoubleTapSprint) {
         container.getExecutor().playAnimationSynchronized(EFNMurasamaAnimations.HF_MURASAMA_RUN_COMBAT_2, 0.0F);
