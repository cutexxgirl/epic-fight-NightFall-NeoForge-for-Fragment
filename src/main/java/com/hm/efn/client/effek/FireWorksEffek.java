...
@OnlyIn(Dist.CLIENT)
public class FireWorksEffek {
   public static final ResourceLocation FIREWORKS_EFFEK = ResourceLocation.fromNamespaceAndPath("efn", "fireworks");

   public static void playFireworks(FireWorksEffek.Type type, Level level, double x, double y, double z, float scale) {
      if (level != null) {
         try {
            ...
            EFN.LOGGER.error("Failed to play fireworks effect", e);
         }
      }
   }

   public static Particle createParticleWrapper(
      FireWorksEffek.Type type, ClientLevel level, double x, double y, double z, double dx, double dy, double dz, float radius
   ) {
   ...
   public record Type(ResourceLocation effekId, float intrinsicRadius) {
      public static final FireWorksEffek.Type LEVEL1 = new FireWorksEffek.Type(FireWorksEffek.FIREWORKS_EFFEK, 0.5F);
   }
}
