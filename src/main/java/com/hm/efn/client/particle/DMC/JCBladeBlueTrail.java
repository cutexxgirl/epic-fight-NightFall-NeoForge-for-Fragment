...
public class JCBladeBlueTrail extends SingleQuadParticle {
   static ResourceLocation texture = EFNRenderType.GetTexture("particle/sparks_blue");
   ...
   public JCBladeBlueTrail(ClientLevel level, double x, double y, double z, double rx, double ry, double rz) {
   ...
   public void render(VertexConsumer buffer, Camera camera, float pt) {
      if (PostEffectPipelines.isActive()) {
         ...
         if (!(at < this.timeOffset) && !(at > this.lifetime + this.timeOffset)) {
            ...
            Vector3f dirOriginal = new Vector3f((float)this.xd, (float)this.yd, (float)this.zd);
            Vector3f dir = new Vector3f(dirOriginal);
            dir.mul(t);
            right.cross(dir);
            right.normalize();
   ...
   public static class Provider implements ParticleProvider<SimpleParticleType> {
      ...
      public Particle createParticle(SimpleParticleType typeIn, ClientLevel worldIn, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
         return new JCBladeBlueTrail(worldIn, x, y, z, xSpeed, ySpeed, zSpeed);
      }
   }
}
